...
+ (void)showMsg:(NSString *)msg inView:(UIView*)theView{
    ...
    WFToastView *alert = [[WFToastView alloc] initWithMsg:msg];
    if (!theView){
        [[WFToastView getUnhiddenFrontWindowOfApplication] addSubview:alert];
    }
    else{
        [[WFToastView getWindow] addSubview:alert];
    }
    [alert showAlert];
...
- (void)showAlert{
    ...
    self.alpha = 0.0;
    CGPoint center = [WFToastView getUnhiddenFrontWindowOfApplication].center;
    //    //调整位置
...
- (id)initWithMsg:(NSString*)_msg{
    ...
    if (self = [super init]) {
        ...
        self.frame = CGRectMake(0, 0, 160, 36 + textSize.height);
